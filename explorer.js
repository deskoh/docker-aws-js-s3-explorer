...
const PROXY_ENDPOINT = 'http://localhost:3030';
...
function ViewController($scope, SharedService) {
  ...
  $tb.on('click', 'a', (e) => {
      ...
      if (target.dataset.s3 === 'folder') {
          ...
          window.open(`${PROXY_ENDPOINT}/${$scope.view.settings.bucket}/${target.dataset.s3key}`, '_blank');
      } else {
...
$(document).ready(() => {
  ...
  AWS.config.update({ endpoint: PROXY_ENDPOINT });
